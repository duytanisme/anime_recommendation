...
if __name__ == "__main__":
    ...
    ratings = spark.read.csv(
        "hdfs://namenode:9000/data/rating_complete.csv", header=True
    )
    ...
    ratings = (
        ratings.withColumn("user_id", col("user_id").cast("integer"))
    ...
    userId_ratings = (
        ratings.groupBy("user_id").count().orderBy("count", ascending=False)
    )
    # userId_ratings.show()

    # Group data by userId, count ratings
    movieId_ratings = (
        ratings.groupBy("anime_id").count().orderBy("count", ascending=False)
    )
    ...
    model.write().overwrite().save(
        f"hdfs://namenode:9000/model/model_{rank}_{regparam}"
    )
    model = CrossValidatorModel.read().load(
        f"hdfs://namenode:9000/model/model_{rank}_{regparam}"
    )
    best_model = model.bestModel
    ...
    nrecommendations = nrecommendations.withColumn(
